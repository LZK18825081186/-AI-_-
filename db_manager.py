...
class DBManager:
    ...
    def init_db(self):
        ...
        try:
            ...
            # 添加砍价底线字段（如果不存在）
            try:
                cursor.execute("ALTER TABLE ai_reply_settings ADD COLUMN bargain_bottom_price REAL DEFAULT 0")
            except:
                pass
            try:
                cursor.execute("ALTER TABLE ai_reply_settings ADD COLUMN bargain_strategy TEXT DEFAULT 'balanced'")
            except:
                pass
        ...
        with self.lock:
            try:
                ...
                (cookie_id, ai_enabled, model_name, api_key, base_url,
                 ...
                 custom_prompts, bargain_bottom_price, bargain_strategy, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
                ''', (
                    cookie_id,
                    ...
                    settings.get('bargain_bottom_price', 0),
                    settings.get('bargain_strategy', 'balanced')
                ))
                self.conn.commit()
        ...
        with self.lock:
            try:
                ...
                SELECT ai_enabled, model_name, api_key, base_url,
                       max_discount_percent, max_discount_amount, max_bargain_rounds,
                       custom_prompts, bargain_bottom_price, bargain_strategy
                ...
                if result:
                    return {
                        ...
                        'bargain_bottom_price': result[8] or 0,
                        'bargain_strategy': result[9] or 'balanced'
                    }
                else:
                    # 返回默认设置
                    return {
                        ...
                        'custom_prompts': '',
                        'bargain_bottom_price': 0,
                        'bargain_strategy': 'balanced'
                    }
            except Exception as e:
