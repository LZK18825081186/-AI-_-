...
class ImageUploader:
    ...
    def _compress_image(self, image_path: str, max_size: int = 5 * 1024 * 1024, quality: int = 85) -> Optional[str]:
        ...
        try:
            ...
            logger.warning(f"图片压缩失败（将使用原文件上传）: {e}")
            return image_path  # 非图片格式（如.mp4），跳过压缩，直接传原文件
    ...
    async def upload_image(self, image_path: str) -> Optional[str]:
        ...
        try:
            ...
            if not temp_path:
                logger.error("文件处理失败")
                return None
            is_compressed = (temp_path != image_path)
            
            # 读取压缩后的图片数据
            with open(temp_path, 'rb') as f:
                image_data = f.read()
            
            # 构造文件名（仅压缩后的图片统一改 .jpg，原文件保留原名）
            filename = os.path.basename(image_path)
            if is_compressed and not filename.lower().endswith(('.jpg', '.jpeg')):
            ...
            # 清理临时压缩文件（不删除原始文件）
            if temp_path and is_compressed and os.path.exists(temp_path):
                try:
                    os.remove(temp_path)
                except:
                    pass
